...
torch.save(mine.T, "minesqfull" + lam_str + ".pth")
np.save("misqfull" + lam_str + ".npy", mi.detach().cpu().numpy())
np.save("losssqfull" + lam_str + ".npy", torch.stack(loss_log).detach().cpu().numpy())

plt.figure()
plt.plot(torch.stack(loss_log).detach().cpu().numpy())
plt.title("sqfull loss: 1000 epochs, lambda=" + lam_str)
plt.ylabel("loss")
plt.xlabel("batches")
plt.savefig("losssqfull" + lam_str+".pdf")
...
for i in range(3):
    ...
    ax.set_aspect("equal")
    plt.colorbar()
    plt.title(f"sqfull, lambda={lam_str}, channel {i}")
    plt.savefig(f"Tweightsc{i}{lam_str}sqfull.pdf")
...
plt.title(f"sqfull, lambda={lam_str}, combined channels")
ax = plt.gca()
ax.set_aspect("equal")
plt.savefig("Tweightscomb" + lam_str + "sqfull.pdf")
...
ax = plt.gca()
ax.set_aspect('equal')
plt.savefig("flat weights" + lam_str + "sqfull.pdf")
