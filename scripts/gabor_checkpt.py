...
mi, loss_list = mine.optimize(torch.tensor(images_flat, dtype=torch.float32), torch.tensor(responses, dtype=torch.float32), 150, batch_size, lam, "test")

torch.save(mine.T, "minegabortest" + lam_str + ".pth")
np.save("migabortest" + lam_str + ".npy", mi.detach().cpu().numpy())
np.save("lossgabortest" + lam_str + ".npy", torch.stack(loss_list).detach().cpu().numpy())

plt.figure()
plt.plot(torch.stack(loss_list).detach().cpu().numpy())
plt.title("gabortest loss: 250 epochs, lambda=" + lam_str)
plt.ylabel("loss")
plt.xlabel("batches")
plt.savefig("lossgabortest" + lam_str+".pdf")
...
for i in range(3):
    ...
    ax.set_aspect("equal")
    plt.colorbar()
    plt.title(f"gabortest, lambda={lam_str}, channel {i}")
    plt.savefig(f"Tweightsc{i}{lam_str}gabortest.pdf")
...
plt.title(f"gabortest, lambda={lam_str}, combined channels")
ax = plt.gca()
ax.set_aspect("equal")
plt.savefig("Tweightscomb" + lam_str + "gabortest.pdf")
